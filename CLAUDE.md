...
- `.docs/decisions/` — **WHY**: Architecture Decision Records, named by topic (`no-rls.md`, `frontmatter-type.md`) — short, human-readable, two words max, no number prefix. One decision per file (context → decision → consequences → source). Cross-reference an ADR by its title, not a number. **Write an ADR for every non-obvious technical choice** so it isn't silently reverted later.
...
- `.docs/product/` — **WHAT**: product, requirements, domain context. `concept.md` is the canonical "what is this app"; discovery lives in `requirements.md` + `discovery.json`; `_input/` = raw client source. Internal commercial (pricing, strategy) lives here too — not for client eyes.
- `.docs/research/` — investigations and findings: market/competitor scans, tech/library evaluations, spike write-ups, comparisons, background reading. **Always write research documents here** — not in `engineering/` or `product/`. Research informs decisions; once a choice is made, the conclusion graduates to an ADR in `decisions/` or a spec in `engineering/`, and the research doc stays as the supporting evidence.
...
**Per-domain current-state JSON.** A domain may keep a machine-readable snapshot mirroring `engineering/architecture.json`'s `meta` block — e.g. `engineering/architecture.json` (the system) and `product/discovery.json` (requirements). Keep them current alongside the prose.

**File naming.** Lowercase kebab-case, and **keep names short — two words max wherever possible** (`drone-regs.md`, `auth-spike.md`, not `commercial-drone-regulations-overview-europe.md`). Let the folder carry the context, so the filename only needs the topic. **The date goes in front only in `reviews/`** — point-in-time audits are `YYYY-MM-DD-short-name.md` (ISO date first, so repeat audits of an area sort chronologically and never collide); the descriptive part still stays to ~two words. Everywhere else, plain short names with no date. ADRs follow the same rule — a topic name, no number prefix (`decisions/no-rls.md`, not `decisions/0001-no-rls.md`). No progress-tracking files.
