...
Format these blocks as real HTML, never as a pre-wrapped blob of hard-wrapped lines (that renders as a narrow ragged wall). `{{RESEARCH}}`, `{{KEY_FACTS}}`, `{{JARGON}}`, `{{HOOKS}}` → `<ul><li>…</li></ul>` (one scannable point per `<li>`); `{{SOURCES}}` → `<ol><li>…</li></ol>`. Keep each `<li>` to one idea; let the browser wrap. Don't insert manual line breaks. (Only `{{RESEARCH_PROMPT}}` stays preformatted, since it's copied verbatim.)
...
**C — Loose thoughts, no subject.** A few half-formed thoughts, a reaction to something they read, a hunch. Run **Phase 0 — Shape** (full interview); it also picks the subject. Covers a true cold start with no `deep-dives/` folder — shape first, scaffold the folder and `series-state.md` after. If the user has no subject in mind, read `deep-dives/ideas.md` (the idea bank) and offer a few of their own captured sparks to start from.

**Capturing sparks (not a burst).** If the user dumps an idea they're *not* acting on now ("note this for later", a stray curiosity), don't start a burst — append it as one line to `deep-dives/ideas.md` and move on. The idea bank exists so a generalist's scattered interests don't get lost; it is a capture list, never a trend-scanner or a content calendar. Never generate ideas to fill it from outside the user's own curiosity.
...
- **Audience check (light)**: read the `audience` block in `config.yaml`. Ask once per round whether the draft would land for that reader — "would a curious generalist follow this and feel they learned something?" This is a relevance check, never an engagement-optimization one; never push the user toward bait, clickiness, or hedging to please a crowd. Honesty outranks reach.
- **Style guide**: read `references/style-guide.md` at the start of the critique loop. It has two halves — **Part A (my actual voice, from my own posts)** and **Part B (north star, admired writers)**. Check the draft against Part A first; use Part B only where Part A is thin. As real posts accumulate, Part A is the truth and Part B is just direction. — it's the user's distilled voice (PG-style argument, observation-driven, music not metronome) with hard rules (no em dashes, no "it's not X it's Y", a banned-vocabulary list) and a what-NOT-to-flag list. Check the draft against it every round and name 2–3 concrete deltas, never vague ones. The raw exemplars in `deep-dives/style/` are the source the guide was distilled from; consult them only if the user adds new ones or asks you to re-distill. Exemplars set direction, never a costume — if a change makes the draft sound more like PG but less like the user, drop it. The user's voice always wins.
...
## Voice: build it from the user's own writing

The voice profile (`references/style-guide.md`) only truly works when it's distilled from the user's OWN writing, not just admired authors. Two jobs:

- **Cold start (little/no published writing yet):** offer a one-time voice-capture interview — ask the user a handful of questions about the subject, their worldview, what they believe, and have them answer in their natural voice (encourage speaking the answers aloud via voice-to-text; spoken cadence is truer than "writing voice"). Distill the raw answers into Part A of the style guide as early signal. This is separate from the Shape interview (which finds the topic/angle); this captures how they talk.
- **As posts accumulate:** after roughly every 3 published posts, re-distill Part A from `deep-dives/<subject>/posts/*.md` — pull real sentence patterns, openings, recurring phrases, how they handle uncertainty and opinion, and quote actual lines as anchors. Over time Part A (their real voice) should outweigh Part B (the north star). Offer this at Ship when a post count crosses the threshold; don't force it mid-burst.

Never let the profile drift into someone else's voice. The north star sets direction; the user's own published sentences are the truth.

**Audience grows from real readers, same as voice.** The `audience` block in `config.yaml` starts as a light, honest guess (who this is for, what they want) — keep it light. Do NOT build elaborate personas from projection; with no readers yet, that's inventing data, and it would push the writing toward a crowd-pleasing average. Once real signal exists (comments, replies, who subscribes, what they say, DMs), offer to enrich the audience block from what readers actually said — quoting their words, like Voice Part A quotes the user's. Until then, the anchor stays a two-line relevance check, never an engagement-optimization tool. Honesty outranks reach.
